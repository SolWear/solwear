package com.solwear.mobile.terminal.nfc

import android.nfc.cardemulation.HostApduService
import android.nfc.NdefMessage
import android.os.Bundle
import android.util.Log
import com.solwear.mobile.terminal.crypto.TerminalKeyManager
 ...
 *
 * The phone (SolWear) reads NDEF from this service to get the wallet pubkey.
 * For signing, the phone writes a sign_request via UPDATE BINARY, then reads
 * back the sign_response on the next tap.
 */
class SolWearHostApduService : HostApduService() {
        ...
        try {
            ...
            for (record in ndefMessage.records) {
                ...
                if (record.tnf == android.nfc.NdefRecord.TNF_EXTERNAL_TYPE &&
                    (recordType.equals("solwear:wallet", ignoreCase = true) ||
                        recordType.equals("solvare:wallet", ignoreCase = true) ||
                        recordType.equals("wallet", ignoreCase = true) ||
                        recordType.endsWith(":wallet", ignoreCase = true))
                ) {
                ...
                if (record.tnf == android.nfc.NdefRecord.TNF_EXTERNAL_TYPE &&
                    (recordType.equals("solwear:sign_response", ignoreCase = true) ||
                        recordType.equals("solvare:sign_response", ignoreCase = true) ||
                        recordType.equals("sign_response", ignoreCase = true) ||
                        recordType.endsWith(":sign_response", ignoreCase = true))
                ) {
                    val payload = String(record.payload).trim()
                ...
                if (record.tnf == android.nfc.NdefRecord.TNF_EXTERNAL_TYPE &&
                    (recordType.equals("solwear:sign_request", ignoreCase = true) ||
                        recordType.equals("solvare:sign_request", ignoreCase = true) ||
                        recordType.equals("sign_request", ignoreCase = true) ||
                        recordType.endsWith(":sign_request", ignoreCase = true))
                ) {
    ...
    private fun buildSignResponseNdefFile(signatureBase64: String): ByteArray {
        ...
        val record = android.nfc.NdefRecord.createExternal("solwear", "sign_response", responsePayload.toByteArray())
        val message = android.nfc.NdefMessage(arrayOf(record))
        val messageBytes = message.toByteArray()
    ...
    companion object {
        private const val TAG = "SolWearHCE"
        ...
        @Volatile
        private var activeInstance: SolWearHostApduService? = null
